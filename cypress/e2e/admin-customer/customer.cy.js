  ...
  it("Register account successfully", () => {
    cy.get("@user").then((user) => {
      registerPage
        .typeName("Mỹ Hà")
        .typeUsername(
          user.register[0].username + `${Math.floor(Math.random() * 100) + 2}`
        )
        .typeEmail(`hangocmy${Math.floor(Math.random() * 100) + 2}@gmail.com`)
...
describe("MODULE ADMIN CUSTOMER", () => {
  ...
  it("Delete customer successfully", () => {
    cy.visit(common.LNK_CUSTOMER).wait(500);
    adminCustomerPage.clickDelete().clickConfirmDetele();
  });
});
